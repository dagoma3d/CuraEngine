...
, fill_pattern_for_all_layers(get_constructor, *density_provider, fractal_config.aabb.getAABB(), fractal_config.depth, line_width, use_dithering)
{
}
...
: fractal_config(getFractalConfig(aabb_3d, min_line_distance, true))
, density_provider(new CombinedDensityProvider(new ImageBasedDensityProvider(cross_subdisivion_spec_image_file, aabb_3d.getAABB()), aabb_3d))
, subdivision_structure_3d(get_constructor, *density_provider, fractal_config.aabb, fractal_config.depth, line_width)
{
    subdivision_structure_3d->initialize();
    subdivision_structure_3d->createMinimalDensityPattern();
